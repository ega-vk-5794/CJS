...
| [`doc/controls/gis-map.md`](doc/controls/gis-map.md) | the parcel map — and the **correction at the top of that file**: the deployed My Properties screen renders the ArcGIS view **inside an iframe** on the GIS host's own origin, which is why the live map has no proxy/CORS problem and CJS's in-page rebuild cannot get past one. An earlier reading of `util/Map.js` concluded the opposite; the DOM overrules it. |
