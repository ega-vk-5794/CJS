...
**19 journeys - 19 tickets - 172 observations - 24 closed - 148 open**
...
| 2 | Applicant details not aligned - should be row wise | Config - developer | Studio > Design tab. Place the applicant fields on one row of the 12-column grid (ZRAK_CJ_LAY). |
...
| 4 | Permit / EID / trade licence order wrong, no search button | Config - developer | Field order in the Design tab; the search button appears when the field's FTYPE is SEARCH. |
| 5 | Company details field order not correct | Config - developer | Design tab. |
...
| 7 | Grid: all row fields mandatory, drop the delete1 column | Config - developer | REQUIRED per field, and remove the column from the grid spec in ZRAK_T_JNY_FLD-DEFAULT_VAL. |
| 8 | Units and No. of Months should be dropdowns | Config - developer | Add the values to ZRAK_T_JNY_OPT and set CLOSED_LIST = X. |
| 9 | Supplier company: order and mandatory indicators | Config - developer | Design tab + REQUIRED. |
| 10 | Step name 'Letter from the Supplier Company' -> 'Documents' | Config - developer | Already coded in ZRAK_CJ_FIXPACK, English and Arabic. Run the report for this journey. |
| 11 | Mandatory attachment labels need the * indicator | Config - developer | The engine draws the asterisk now - RENDER_ATTACH( ) passes the native required property. What is left is ticking REQUIRED on each upload field. |
| 12 | Declaration & Notes missing | Config - developer | Add a DISPLAY field whose DEFAULT_VAL is TEXT:@nnn pointing at a ZRAK_T_CJ_TXT row, so it reads in both languages. |
| 13 | Feedback is mandatory | Framework - we push | A 'Not now' button now sits beside Send feedback and the journey finishes without it. ZCL_RAK_JOURNEY_RENDER + ZCL_RAK_JOURNEY_ENGINE (FBSKIP). |
...
| 15 | Arabic - service start button not showing | Portal / launchpad | Service card / launchpad, not CJS. |
...
| 2 | Applicant details not aligned - should be row wise | Config - developer | Design tab. |
| 3 | Applicant details are duplicated | Handler class - developer | The handler was writing the applicant fields twice. Fixed in ZCL_EPDA_E022_DEV_PROJ_LOGIC. |
| 4 | Emirates ID search should follow the format; Browse not required | Config - developer | PLACEHOLDER for the mask, REGEX for the format, FTYPE SEARCH instead of the browse control. |
| 5 | Permit / EID / trade licence order; permit number not a dropdown | Config - developer | Design tab + FTYPE. |
| 6 | Company details field order not correct | Config - developer | Design tab. |
| 7 | Company name English & Arabic duplicated | Config - developer | Two fields are bound to the same value. Hide one in ZRAK_T_JNY_FLD. |
| 8 | Rename step 'Development Project' -> 'Project Details' | Config - developer | Coded in ZRAK_CJ_FIXPACK, EN + AR. |
| 9 | Rename step 'Trade license' -> 'Documents' | Config - developer | Coded in ZRAK_CJ_FIXPACK, EN + AR. |
| 10 | Trade licence attachment is missing | Config - developer | Add the upload field to the Documents step. |
| 11 | Declaration & Note missing | Config - developer | DISPLAY field with a TEXT: default. |
| 12 | Step 5 request confirmation not required | Config - developer | Coded in ZRAK_CJ_FIXPACK as STEP_OFF - reversible, sets ACTIVE blank. |
| 13 | Feedback is mandatory | Framework - we push | 'Not now' button - same engine fix as E021. |
| 14 | Attachments are not saving | Needs a look | Re-run with &trace=x and send the ATTACH line. |
| 15 | Customer action not working | Portal / launchpad | Landing page redirect - portal team. |
...
| 4 | Customer Action from landing page, Upload does nothing | Portal / launchpad | Framework side - Jyoti's answer. |
| 5 | Cases not translated into Arabic | Portal / launchpad | Framework side. |
| 6 | No case exists to submit in Arabic | Portal / launchpad | Framework side. |
| 7 | Back button does not work to submit a new case | Portal / launchpad | Framework side. |
...
EPDA · CJSMIG-687 · SIT · Jyoti Meena · blocks CJSMIG-522 · 7 screenshots on the ticket · **18 observations, 3 closed, 15 open**
...
| 6 | One field per row - should be two or more | Framework - we push | Popup dialogs lay out in two columns now. The fix was FormContainers, not the columnsL property. ZCL_RAK_JOURNEY_LOGIC->DIALOG_FORM( ). |
...
| 9 | HS code should be a guided formatted number with a watermark | Config - developer | REGEX for the format, PLACEHOLDER for the watermark. |
...
| 12 | 'up to 2 MB' repeats on every attachment; mandatory indicator missing | Config - developer | The asterisk is engine-side and done; the repeated size hint is per-field wording. |
| 13 | Preview page should be removed | Config - developer | Coded in ZRAK_CJ_FIXPACK as STEP_OFF. |
| 14 | Step 5 request confirmation not required | Config - developer | Coded in ZRAK_CJ_FIXPACK as STEP_OFF. |
| 15 | Feedback is mandatory | Framework - we push | 'Not now' button. |
| 16 | Attachments are not saving | Needs a look | Re-run with &trace=x. |
| 17 | Customer action not working | Portal / launchpad | Portal team. |
...
| 6 | Arabic translation not complete | Config - developer | Handed to anand on the thread. ZLABEL_AR across the journey. |
...
| 10 | Customer Action should redirect to the new CJS | Portal / launchpad | Portal team. |
| 11 | Mandatory attachments need the * | Config - developer | Engine draws it; tick REQUIRED on each upload field. |
...
| 4 | Customer Action upload not working | Portal / launchpad | Framework side - Jyoti's answer. |
| 5 | Cases not translated into Arabic | Portal / launchpad | Framework side. |
| 6 | No case exists to submit in Arabic | Portal / launchpad | Framework side. |
| 7 | Back button does not work to submit a new case | Portal / launchpad | Framework side. |
| 8 | Applicant type field alignment | Closed by developer | Closed by Jyoti. |
...
| 6 | Customer Action upload not working | Portal / launchpad | Framework side. |
| 7 | Old case journey portal not working, cannot compare | Portal / launchpad | Portal team. |
| 8 | Cases not translated into Arabic | Portal / launchpad | Framework side. |
| 9 | No case exists to submit in Arabic | Portal / launchpad | Framework side. |
| 10 | Back button does not work to submit a new case | Portal / launchpad | Framework side. |
...
| # | Observation | Status | What closes it |
| ---: | --- | --- | --- |
| 1 | Owner - BP labels missing | Config - developer | Fill ZLABEL / ZLABEL_AR. In the fixpack worklist. |
| 2 | Owner details missing from the application form | Backend / record model | Record model. |
| 3 | Arabic version has no Start button | Portal / launchpad | Service card. |
| 4 | Customer Action should redirect to CJS | Portal / launchpad | Portal team. |
| 5 | Mandatory attachments need the * | Config - developer | Tick REQUIRED on each upload field. |
...
| # | Observation | Status | What closes it |
| ---: | --- | --- | --- |
| 1 | Field should read Distance, not Educational stage | Config - developer | ZLABEL / ZLABEL_AR. In the fixpack worklist - the field name is not known from here. |
...
| # | Observation | Status | What closes it |
| ---: | --- | --- | --- |
| 1 | Arabic version has no Start button | Portal / launchpad | Service card. Reassigned to anand. |
| 2 | Mandatory attachments need the * | Config - developer | Tick REQUIRED on each upload field. |
| 3 | Customer Action should redirect to CJS | Portal / launchpad | Portal team. |
...
| 2 | Arabic version has no Start button | Portal / launchpad | Service card. |
| 3 | Mandatory attachments need the * | Config - developer | Tick REQUIRED on each upload field. |
| 4 | Customer Action should redirect to CJS | Portal / launchpad | Portal team. |
...
| 6 | One field per row - should be two or more | Framework - we push | Two-column dialogs - engine fix. |
...
| # | Observation | Status | What closes it |
| ---: | --- | --- | --- |
| 1 | Arabic version has no Start button | Portal / launchpad | Service card. |
| 2 | Mandatory attachments need the * | Config - developer | Tick REQUIRED on each upload field. |
| 3 | Customer Action should redirect to the new CJS | Portal / launchpad | Portal team. |
...
| # | Observation | Status | What closes it |
| ---: | --- | --- | --- |
| 1 | Service card is missing the 'who can apply' information | Portal / launchpad | Service catalogue - taken by Vikram on the thread. |
...
| 1 | Initial page loads with error messages | Needs a look | The screenshot is on the ticket; paste it here - Jira is not reachable from where this register is built. |
| 2 | Applicant details not aligned - should be row wise | Config - developer | Design tab. |
| 3 | Applicant details are duplicated | Config - developer | Not the same cause as E022 - this handler writes nothing. Two fields are bound to the same value; hide one. |
| 4 | Emirates ID search should follow the format; Browse not required | Config - developer | PLACEHOLDER + REGEX; FTYPE SEARCH. |
| 5 | Permit / EID / trade licence order; permit number not a dropdown | Config - developer | Design tab + FTYPE. |
| 6 | Company details field order not correct | Config - developer | Design tab. |
| 7 | Company name English & Arabic duplicated | Config - developer | Hide one of the two fields. |
| 8 | 'Material Text 1' should read 'Name' | Config - developer | ZLABEL / ZLABEL_AR. |
| 9 | Issuing emirates should be a dropdown | Config - developer | ZRAK_T_JNY_OPT + CLOSED_LIST. |
| 10 | Vehicle Code must not allow more than 2 digits | Config - developer | MAX_LEN on the field. |
| 11 | Vehicle Plate must not allow more than 4 digits | Config - developer | MAX_LEN on the field. |
| 12 | Remove the 'Mtable Delete 2' column | Config - developer | Drop it from the grid spec in DEFAULT_VAL. |
| 13 | Units should be a dropdown | Config - developer | ZRAK_T_JNY_OPT + CLOSED_LIST. |
| 14 | Address, Registered Emirate, Trade Licence should be inputs, not display | Config - developer | Change FTYPE from DISPLAY to INPUT and tick REQUIRED. |
| 15 | Step name should change to Documents | Config - developer | Step title, EN + AR. |
| 16 | Mandatory attachments need the * | Config - developer | Tick REQUIRED on each upload field. |
| 17 | Declaration & Notes missing | Config - developer | DISPLAY field with a TEXT: default. |
| 18 | Feedback is mandatory | Framework - we push | 'Not now' button. |
...
| 22 | Customer action | Portal / launchpad | Portal team. |
| 23 | Arabic services cannot be submitted | Portal / launchpad | Framework side. |
...
| 1 | Initial page loads with error messages | Needs a look | The screenshot is on the ticket; paste it here - Jira is not reachable from where this register is built. |
| 2 | Applicant details not aligned - should be row wise | Config - developer | Design tab. |
| 3 | Applicant details are duplicated | Config - developer | Two fields bound to the same value; hide one. |
| 4 | Emirates ID search should follow the format; Browse not required | Config - developer | PLACEHOLDER + REGEX; FTYPE SEARCH. |
| 5 | Permit / EID / trade licence order; permit number not a dropdown | Config - developer | Design tab + FTYPE. |
| 6 | Rename the step 'Material Origin source' to 'Batteries/Scrap Details' | Config - developer | Step title, EN + AR. |
| 7 | Fields under Material origin source must be dynamic and mandatory by dropdown value | Config - developer | Show/hide belongs in ZRAK_T_JNY_RULE keyed on the dropdown, with REQUIRED on the fields it reveals. |
| 8 | Vehicle code 1 digit, vehicle plate 4 digits | Config - developer | MAX_LEN on both fields. |
| 9 | 'Materials Det' is duplicated and should be renamed | Config - developer | Hide one, rename the other. |
| 10 | Declaration & Note missing | Config - developer | DISPLAY field with a TEXT: default. |
| 11 | Step 5 request confirmation not required | Config - developer | STEP_OFF - the fixpack does this for other journeys and takes one more line. |
| 12 | Feedback is mandatory | Framework - we push | 'Not now' button. |
...
| # | Observation | Status | What closes it |
| ---: | --- | --- | --- |
| 1 | CJS service Not found | Config - developer | The handler class exists and is complete. Check the ZRAK_T_JNY row and the launch URL's journey id - a code that does not resolve gives exactly this. |
...
| # | Observation | Status | What closes it |
| ---: | --- | --- | --- |
| 1 | Emirates ID search should follow the format; Browse not required | Config - developer | PLACEHOLDER + REGEX; FTYPE SEARCH. |
| 2 | Number of Apiaries max 2 digits; Expected Production max 6 | Config - developer | MAX_LEN on both fields. |
| 3 | Mandatory attachments need the * | Config - developer | Tick REQUIRED on each upload field. |
| 4 | Declaration & Note missing | Config - developer | DISPLAY field with a TEXT: default. |
...
| 6 | Applicant BP not saved in the record model, not in the backend application | Handler class - developer | Fixed. The class called the applicant field OWNER_BP, which is this journey's owner SEARCH BOX; ON_INIT wrote the partner there and blanked it sixteen lines later. Now goes to LOGIN_BP. ZCL_E025_BEEKEEPING_LOGIC. |
...
| 1 | Emirates ID search should follow the format; Browse not required | Config - developer | PLACEHOLDER + REGEX; FTYPE SEARCH. |
| 2 | Mandatory attachments need the * | Config - developer | Tick REQUIRED on each upload field. |
| 3 | Declaration & Note missing | Config - developer | DISPLAY field with a TEXT: default. |
| 4 | Case id is not correct - shows the wrong number | Needs a look | Same as E025 - send the pair of numbers. |
| 5 | Applicant BP not saved in the record model, not in the backend application | Handler class - developer | The applicant partner now also goes out as LOGIN_BP, the field every other journey uses. ZCL_E026_TREE_REMOVAL_LOGIC. |
