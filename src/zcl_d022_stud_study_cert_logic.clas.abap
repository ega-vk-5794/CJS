class ZCL_D022_STUD_STUDY_CERT_LOGIC definition
  public
...
CLASS ZCL_D022_STUD_STUDY_CERT_LOGIC IMPLEMENTATION.
...
*    CHECK to_upper( iv_field ) = 'STUDENTSEARCH'.
*
*    DATA(lv_eid) = condense( io_ctx->get_val( 'STUDENTSEARCH' ) ).
*
*    DATA: lv_emirates_id TYPE bu_id_number, " OPTIONAL,
*          lv_student_id   TYPE char12. " OPTIONAL
*
*    DATA(lv_idtype) = io_ctx->get_val( 'STUDENTSEARCH_IDTYPE' ).
...
*    ELSEIF lv_idtype = 'YFS003'.
*      lv_student_id = lv_eid.
*    ENDIF.
**    DATA(lt_student1) = zcl_cj_moe_api=>get_student_v1( iv_emirates_id = lv_eid iv_student_id = '2013196053' ).
...
*
**  DATA(lv_academic_year) = COND #( WHEN iv_academic_year IS NOT INITIAL THEN iv_academic_year
**                                   ELSE VALUE #( lt_student[ 1 ]-academic_year OPTIONAL ) ).
*    io_ctx->set_val( iv_name = 'STUDENT_NAME'        iv_value = |{ ls_student-full_name_en }| ).
*    io_ctx->set_val( iv_name = 'STUDENT_EMIRATESID'        iv_value = |{ ls_student-nationalid }| ).
*    io_ctx->set_val( iv_name = 'STUDENT_SIS_ID'      iv_value = |{ ls_student-studentid }| ).
*    io_ctx->set_val( iv_name = 'STUDENT_SCHOOL'       iv_value = |{ ls_student-school_name_en }| ).
*    io_ctx->set_val( iv_name = 'STUDENT_YEAR'         iv_value = |{ ls_student-academic_year }| ).
*    io_ctx->set_val( iv_name = 'STUDENT_CYCLE' iv_value = |{ ls_student-cycle }| ).
**
** |{ ls_student-STUDENT_NAME }| ).
*    io_ctx->set_val( iv_name = 'STUDENT_GRADE'  iv_value = |{ ls_student-gradelevelid }| ).
*    io_ctx->set_val( iv_name = 'STUDENT_CURRICULUM'        iv_value = |{ ls_student-curriculum_english }| ).
*    io_ctx->set_val( iv_name = 'STUDENT_PHONE'      iv_value = |{ ls_student-phone_number }| ).
*    io_ctx->set_val( iv_name = 'STUDENT_NATIONALITY'       iv_value = |{ ls_student-nationality_en }| ).
*    io_ctx->set_val( iv_name = 'STUDENT_EMAIL'         iv_value = |{ ls_student-student_email }| ).
