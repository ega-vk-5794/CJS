*---------------------------------------------------------------------*
*    program for:   VIEWFRAME_ZMV_JNY_OPT
*---------------------------------------------------------------------*
FUNCTION VIEWFRAME_ZMV_JNY_OPT         .
...
*----------------------------------------------------------------------*
CALL FUNCTION 'VIEWPROC_ZMV_JNY_OPT'
         EXPORTING
              FCODE          = READ
...
EXTRACT = ZMV_JNY_OPT_EXTRACT
TOTAL = ZMV_JNY_OPT_TOTAL
              X_HEADER       = X_HEADER
              X_NAMTAB       = X_NAMTAB
...
CALL FUNCTION 'VIEWPROC_ZMV_JNY_OPT'
             ...
             IMPORTING
                  UCOMM           = FUNCTION
UPDATE_REQUIRED = STATUS_ZMV_JNY_OPT-UPD_FLAG
             TABLES
                  EXCL_CUA_FUNCT  = EXCL_CUA_FUNCT
EXTRACT = ZMV_JNY_OPT_EXTRACT
TOTAL = ZMV_JNY_OPT_TOTAL
...
*----------------------------------------------------------------------*
      ...
      IF ( FUNCTION EQ SWITCH_TO_SHOW_MODE OR
           ...
           FUNCTION EQ END ) AND
STATUS_ZMV_JNY_OPT-UPD_FLAG NE SPACE.
        PERFORM BEENDEN.
        CASE SY-SUBRC.
          WHEN 0.
CALL FUNCTION 'VIEWPROC_ZMV_JNY_OPT'
...
TOTAL = ZMV_JNY_OPT_TOTAL
            ...
            CASE SY-SUBRC.
              WHEN 0.
IF STATUS_ZMV_JNY_OPT-UPD_FLAG EQ SPACE. EXIT. ENDIF.
...
IF STATUS_ZMV_JNY_OPT-UPD_FLAG NE SPACE.
