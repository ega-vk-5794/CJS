*---------------------------------------------------------------------*
*    program for:   VIEWFRAME_ZMV_JNY_STEP
*---------------------------------------------------------------------*
FUNCTION VIEWFRAME_ZMV_JNY_STEP        .
...
*----------------------------------------------------------------------*
CALL FUNCTION 'VIEWPROC_ZMV_JNY_STEP'
         EXPORTING
              FCODE          = READ
...
EXTRACT = ZMV_JNY_STEP_EXTRACT
TOTAL = ZMV_JNY_STEP_TOTAL
              X_HEADER       = X_HEADER
              X_NAMTAB       = X_NAMTAB
...
CALL FUNCTION 'VIEWPROC_ZMV_JNY_STEP'
             ...
             IMPORTING
                  UCOMM           = FUNCTION
UPDATE_REQUIRED = STATUS_ZMV_JNY_STEP-UPD_FLAG
             TABLES
                  EXCL_CUA_FUNCT  = EXCL_CUA_FUNCT
EXTRACT = ZMV_JNY_STEP_EXTRACT
TOTAL = ZMV_JNY_STEP_TOTAL
...
*----------------------------------------------------------------------*
      ...
      IF ( FUNCTION EQ SWITCH_TO_SHOW_MODE OR
           ...
           FUNCTION EQ END ) AND
STATUS_ZMV_JNY_STEP-UPD_FLAG NE SPACE.
        PERFORM BEENDEN.
        CASE SY-SUBRC.
          WHEN 0.
CALL FUNCTION 'VIEWPROC_ZMV_JNY_STEP'
...
TOTAL = ZMV_JNY_STEP_TOTAL
            ...
            CASE SY-SUBRC.
              WHEN 0.
IF STATUS_ZMV_JNY_STEP-UPD_FLAG EQ SPACE. EXIT. ENDIF.
...
IF STATUS_ZMV_JNY_STEP-UPD_FLAG NE SPACE.
